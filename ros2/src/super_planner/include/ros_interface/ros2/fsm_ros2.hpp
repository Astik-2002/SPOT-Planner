...
namespace fsm {
    class FsmRos2 : public Fsm {
        ...
        rclcpp::Publisher<custom_interface_gym::msg::TrajMsg>::SharedPtr gympy_cmd_pub_;
        rclcpp::Publisher<mars_quadrotor_msgs::msg::PolynomialTrajectory>::SharedPtr mpc_cmd_pub_;
        rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr path_pub_;
        rclcpp::Subscription<geometry_msgs::msg::PoseStamped>::SharedPtr goal_sub_;
        rclcpp::Subscription<nav_msgs::msg::Path>::SharedPtr goal_sub_gympy;
        ...
        custom_interface_gym::msg::TrajMsg traj_cmd_;
...
//            ros_ptr_->getSimTime(pos_cmd.header.stamp.sec, pos_cmd.header.stamp.nanosec);
    ...
    public:
        ...
        void goalPathCallback(const nav_msgs::msg::Path::SharedPtr msg) {
            if (msg->poses.empty()) {
                RCLCPP_WARN(nh_->get_logger(), "[Fsm] Received empty Path, ignoring.");
                return;
            }

            const auto &first_pose = msg->poses.front().pose;

            super_utils::Vec3f goal_p{first_pose.position.x,
                                    first_pose.position.y,
                                    first_pose.position.z};

            super_utils::Quatf goal_q{first_pose.orientation.w,
            ...
            RCLCPP_INFO(nh_->get_logger(),
                        "[Fsm] Received Path goal → using first point (%.2f, %.2f, %.2f)",
        ...
        void init(const rclcpp::Node::SharedPtr nh, const std::string &cfg_path) {
            ...
            gympy_cmd_pub_ = nh->create_publisher<custom_interface_gym::msg::TrajMsg>("rrt_command", 10);
            ...
            if (cfg_.click_goal_en) {
                ...
                // Default: subscribe to PoseStamped
                ...
                std::cout << YELLOW << " -- [Fsm] CLICKGOAL ENABLE (PoseStamped)." << RESET << std::endl;
            
                // Gympy mode: subscribe to Path
                goal_sub_gympy = nh_->create_subscription<nav_msgs::msg::Path>(
                    "waypoints",
                    1,
                    std::bind(&FsmRos2::goalPathCallback, this, std::placeholders::_1),
                    so);

                std::cout << YELLOW << " -- [Fsm] gympy goal ENABLE (Path)." << RESET << std::endl;
        ...
        void convertToTrajMsg(const mars_quadrotor_msgs::msg::PositionCommand &src,
                      custom_interface_gym::msg::TrajMsg &dst) {
            ...
            // Snap is not in PositionCommand → set zero
            dst.snap.x = 0.0;
            dst.snap.y = 0.0;
            dst.snap.z = 0.0;
            ...
            // Decide hover condition: e.g., if velocity is near zero
            double vnorm = std::sqrt(src.velocity.x * src.velocity.x +
                                    src.velocity.y * src.velocity.y +
                                    src.velocity.z * src.velocity.z);
            dst.hover = (vnorm < 1e-3);  
        }
        ...
        void pubCmdTimerCallback() {
            ...
            getOnePositionCommand(pid_cmd_, traj_finish_);
            convertToTrajMsg(pid_cmd_, traj_cmd_);

            mpc_cmd_pub_->publish(heartbeat);
            gympy_cmd_pub_->publish(traj_cmd_);
