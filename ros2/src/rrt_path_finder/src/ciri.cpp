...
namespace super_planner {
    ...
    RET_CODE CIRI::comvexDecomposition(const Eigen::MatrixX4d& bd, const Eigen::Matrix3Xd& pc, const Eigen::Vector3d& a,
                                       const Eigen::Vector3d& b, const Eigen::Vector3d &o, bool uncertanity) {
        ...
        for (int loop = 0; loop < iter_num_; ++loop) {
            ...
            for (int i = 0; !completed && i < (M + N); ++i) 
            {
                if (minSqrD < minSqrR) {
                    ...
                    if (robot_rt < epsilon_) {
                        ...
                        if (temp_plane_w.head(3).dot(a) + temp_plane_w(3) > -epsilon_) {
                            ...
                            findTangentPlaneOfSphere(pt_w, robot_rt, a, E.d(), temp_plane_w);
                        } else if (temp_plane_w.head(3).dot(b) + temp_plane_w(3) > -epsilon_) {
                            // Case the plan make seed out, the plane should be modified in world frame
                            findTangentPlaneOfSphere(pt_w, robot_rt, b, E.d(), temp_plane_w);
                        }
                    }
                    pcFlags(pcMinId) = 0;
                ...
                for (int j = 0; j < N; ++j) {
                    if (pcFlags(j)) 
                    {
                        double adjusted_robot_rt = robot_r_;
                        if (uncertanity) 
                        {
                            auto noise_pt_w = noise_pc.col(j);
                            double uncertain_radius = noise_pt_w.maxCoeff() * sqrt(11.3345);
                            adjusted_robot_rt += uncertain_radius;
                        }
                        if ((temp_plane_w.head(3).dot(pc.col(j)) + temp_plane_w(3)) > adjusted_robot_rt - epsilon_) {
                            pcFlags(j) = 0;
                        }
                        else {
                            completed = false;
